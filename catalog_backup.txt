          ...
          {adminUser && (
            <div style={{ background: 'rgba(200,0,0,0.07)', borderBottom: '1px solid rgba(200,0,0,0.18)', padding: '9px 20px' }}>
              <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                ...
                {activeNav !== 'fabric' && <button className="btn-red sm" onClick={() => setShowAdd(true)}>+ เน€เธเธดเนเธกเนเธเธเน€เธชเธทเนเธญ</button>}
                ...
                {activeNav === 'fabric' && <button className="btn-red sm" onClick={() => setShowFabricAdd(true)}>๐งต เน€เธเธดเนเธกเน€เธเธทเนเธญเธเนเธฒ</button>}
          ...
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 20px' }}>
            ...
            {activeNav === 'fabric' && adminUser && fabrics.length === 0 && (
              <div style={{ textAlign: 'center', padding: '70px 20px' }}>
                <div style={{ fontSize: 50, marginBottom: 16, opacity: .2 }}>๐งต</div>
                <div style={{ color: 'rgba(255,255,255,0.22)', fontSize: 14, marginBottom: 18 }}>เธขเธฑเธเนเธกเนเธกเธตเน€เธเธทเนเธญเธเนเธฒเนเธเธฃเธฐเธเธ</div>
                <button className="btn-red" style={{ padding: '10px 30px' }} onClick={() => setShowFabricAdd(true)}>๐งต เน€เธเธดเนเธกเน€เธเธทเนเธญเธเนเธฒเนเธฃเธ</button>
              </div>
            )}
                ...
                {fabrics.map((fab) => (
                  <FabricCard key={fab.id} fabric={fab} isAdmin={!!adminUser}
                    ...
                    onDelete={async () => {
                      ...
                      notify('เธฅเธเน€เธเธทเนเธญเธเนเธฒเนเธฅเนเธง', 'err')
                    }}
                  />
                ))}
              </div>
            ...
            {activeNav !== 'fabric' && (filtered.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '70px 20px' }}>
                ...
                {adminUser && activeNav !== 'fabric' && <button className="btn-red" style={{ padding: '10px 30px' }} onClick={() => setShowAdd(true)}>+ เน€เธเธดเนเธกเนเธเธเน€เธชเธทเนเธญเนเธฃเธ</button>}
              </div>
            ) : (
              <div className="grid-shirts">
                {filtered.map((s) => (
      ...
      {showFabricAdd && (
        <FabricModal
          onSave={async (data, imgFile) => {
            ...
            if (newFab) { setFabrics((prev) => [newFab, ...prev]); setShowFabricAdd(false); notify('เน€เธเธดเนเธกเน€เธเธทเนเธญเธเนเธฒเนเธฅเนเธง') }
          }}
      ...
      {editFabric && (
        <FabricModal
          ...
          onSave={async (data, imgFile) => {
            ...
            if (updated) { setFabrics((prev) => prev.map((x) => x.id === editFabric.id ? updated : x)); setEditFabric(null); notify('เธเธฑเธเธ—เธถเธเนเธฅเนเธง') }
          }}
          ...
          {category !== 'fabric' ? (
            ...
            <div><div className="section-label">เธเธฃเธฐเน€เธ เธ—เน€เธเธทเนเธญเธเนเธฒ</div>
              <select className="select-d" value={f.product_type || 'เนเธกเนเธเธฃเนเธเธฅเธตเน€เธญเธชเน€เธ•เธญเธฃเน'} onChange={(e) => set('product_type', e.target.value)}>
                <option value="เนเธกเนเธเธฃเนเธเธฅเธตเน€เธญเธชเน€เธ•เธญเธฃเน">เนเธกเนเธเธฃเนเธเธฅเธตเน€เธญเธชเน€เธ•เธญเธฃเน</option>
    ...
    <div className="card-shirt" style={{ position: 'relative' }}>
      <div style={{ aspectRatio: '1', background: '#1a1a1a', position: 'relative', overflow: 'hidden' }}>
        {fabric.image_url
          ...
          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.08)', fontSize: 44 }}>๐งต</div>
        }
      ...
      <div style={{ padding: '13px 14px 12px' }}>
        <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4, color: '#fff', lineHeight: 1.3 }}>{fabric.name || 'เนเธกเนเธกเธตเธเธทเนเธญ'}</div>
        ...
        <div style={{ fontWeight: 700, fontSize: 15, color: Number(fabric.price) > 0 ? '#ff4444' : 'rgba(255,255,255,0.3)' }}>{Number(fabric.price) > 0 ? `+${Number(fabric.price).toLocaleString()} เธเธฒเธ—/เธ•เธฑเธง` : 'เนเธกเนเธเธงเธเน€เธเธดเนเธก'}</div>
        {isAdmin && (
          <div style={{ display: 'flex', gap: 5, marginTop: 10 }}>
            <button className="btn-outline sm" style={{ flex: 1 }} onClick={onEdit}>โ เนเธเนเนเธ</button>
            <button className="btn-ghost" style={{ flex: 1 }} onClick={onDelete}>โ• เธฅเธ</button>
          </div>
        )}
      </div>
    </div>
...
function FabricModal({ initial, onSave, onClose }: {
  ...
  const [f, setF] = useState({
    name: initial?.name || '',
    fabric_type: initial?.fabric_type || 'เนเธกเนเธเธฃเนเธเธฅเธตเน€เธญเธชเน€เธ•เธญเธฃเน',
  ...
  return (
    <div className="modal-bg" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-box">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div style={{ fontWeight: 700, fontSize: 16 }}>{initial ? 'โ เนเธเนเนเธเน€เธเธทเนเธญเธเนเธฒ' : '+ เน€เธเธดเนเธกเน€เธเธทเนเธญเธเนเธฒเนเธซเธกเน'}</div>
          <button className="btn-outline sm" onClick={onClose}>โ• เธเธดเธ”</button>
        </div>

        {/* Image Upload */}
        <div className="section-label">เธฃเธนเธเธ เธฒเธเน€เธเธทเนเธญเธเนเธฒ (เธญเธฑเธเนเธซเธฅเธ”เธชเธนเน Supabase Storage)</div>
        {imgPreview ? (
          <div style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', height: 170, marginBottom: 16 }}>
            <img src={imgPreview} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            <button className="btn-red sm" style={{ position: 'absolute', top: 8, right: 8 }}
              onClick={() => { setImgPreview(null); setNewImgData('__remove__') }}>เน€เธเธฅเธตเนเธขเธเธฃเธนเธ</button>
          </div>
        ) : (
          <div className={`drag-zone${ov ? ' ov' : ''}`} style={{ marginBottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}
            onDragOver={(e) => { e.preventDefault(); setOv(true) }}
            ...
            onDrop={(e) => { e.preventDefault(); setOv(false); if (e.dataTransfer.files[0]) loadImg(e.dataTransfer.files[0]) }}
            onClick={() => ref.current?.click()}>
            <div style={{ fontSize: 28 }}>๐งต</div>
            <div style={{ color: '#c00', fontWeight: 700, fontSize: 13 }}>เธฅเธฒเธ-เธงเธฒเธเธฃเธนเธเธ เธฒเธ เธซเธฃเธทเธญเธเธฅเธดเธเน€เธฅเธทเธญเธ</div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>JPG ยท PNG ยท WEBP โ€” เธญเธฑเธเนเธซเธฅเธ”เธชเธนเน Supabase Storage</div>
          </div>
        )}
        ...
        <div style={{ display: 'grid', gap: 13 }}>
          {/* Name */}
          <div>
            <div className="section-label">เธเธทเนเธญเน€เธเธทเนเธญเธเนเธฒ</div>
            <input className="input-d" value={f.name} onChange={(e) => set('name', e.target.value)} placeholder="เน€เธเนเธ เนเธกเนเธเธฃเนเธเธฅเธตเธฏ เน€เธเธฃเธ” A" />
          </div>

          {/* Fabric Type */}
          <div>
            <div className="section-label">เธเธฃเธฐเน€เธ เธ—เน€เธเธทเนเธญเธเนเธฒ</div>
          ...
          {/* Price */}
          <div>
            <div className="section-label">+เธเธงเธเน€เธเธดเนเธก เธ•เธฑเธงเธฅเธฐ</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {/* Description */}
          <div>
            <div className="section-label">เธเธธเธ“เธชเธกเธเธฑเธ•เธดเน€เธเธทเนเธญเธเนเธฒ</div>
            <textarea className="input-d" value={f.description}
              onChange={(e) => set('description', e.target.value)}
              placeholder="เน€เธเนเธ เธเนเธณเธซเธเธฑเธเธเนเธฒ 150 เนเธเธฃเธก เธฃเธฐเธเธฒเธขเธญเธฒเธเธฒเธจเนเธ”เนเธ”เธต เนเธกเนเธซเธ”เธ•เธฑเธง..."
